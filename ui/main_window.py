...
class MainWindow(QMainWindow):
    ...
    def _on_exchange_added(self, name, exchange_type, params):
        ...
        try:
            ...
            logger.exception("Failed to create exchange %s (%s): %s", name, type_code, exc)
            self.status_bar.show_error(f"{type_code.upper()} {name}: cannot create connector")
            return
        ...
        if params.get("api_key") and params.get("api_secret"):
            try:
                ...
                logger.exception("Failed to connect exchange %s (%s): %s", name, type_code, exc)
                self.status_bar.show_error(f"{type_code.upper()} {name}: invalid API key/format")
    ...
    def _on_status_updated(self, statuses):
        for name, status in statuses.items():
            ...
            if "ошибка" in status_text.lower() or "не реализовано" in status_text.lower():

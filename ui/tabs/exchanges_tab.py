from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QPushButton,
                              QScrollArea, QMessageBox, QComboBox, QLabel)
...
class ExchangesTab(QWidget):
    ...
    def _init_ui(self):
        ...
        self.type_combo.addItem("Binance Futures")
        self.type_combo.addItem("Bitget Futures")
        ...
        scroll.setStyleSheet("QScrollArea { border: 1px solid #2a343c; border-radius: 4px; background-color: #0a0c10; }")
    ...
    def _create_panel(self, name, exchange):
        ...
        elif isinstance(exchange, BitgetExchange):
        ...
        if exchange_type == "Bitget" and hasattr(exchange, 'api_passphrase'):
    ...
    def _add_new_panel(self):
        ...
        exchange_type = self.type_combo.currentText()
    ...
    def _on_new_panel_connect(self, name, params):
        exchange_type = self.type_combo.currentText()
        type_code = "binance" if "Binance" in exchange_type else "bitget"
        
        base_name = "Binance" if "Binance" in exchange_type else "Bitget"
    ...
    def _on_panel_connect(self, name, params):
        ...
        type_code = "binance" if panel.exchange_type == "Binance" else "bitget"
        self.exchange_added.emit(name, type_code, params)
    ...
    def _on_panel_remove(self, name):
        reply = QMessageBox.question(
            self, "Подтверждение", f"Удалить биржу {name}?",
    ...
    def _connect_all(self):
        for name, panel in self.exchange_panels.items():
